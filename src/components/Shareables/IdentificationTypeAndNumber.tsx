...
import { CustomerType } from 'Screens/CustomerCreation'
...
const IdentificationTypeAndNumber = ({ customerType }: Props) => {
    ...
    <div className='flex flex-col gap-8 whitespace-nowrap w-full xl:ml-5 px-8'>
      ...
      <div className='flex gap-10'>
        ...
        <div className='w-full flex justify-between py-2 leading-6 border-b-2 border-[#8F8F8F] text-text-disabled max-w-[318px]'>
          <input type='text' placeholder='Enter text' onChange={(e) => setIdentificationNumber(e.target.value.trim())} />
        </div>
      </div>
    </div>
  )
