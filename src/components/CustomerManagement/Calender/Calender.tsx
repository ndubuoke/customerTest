...
import { add, differenceInDays, endOfMonth, format, getDate, setDate, startOfMonth, sub, subDays, subMonths } from 'date-fns'
    ...
    <div ref={calenderRef} className='w-[450px] h-[350px] border border-line-faint-background  bg-background-paper right-4  absolute z-50 top-8 '>
      <div className='grid grid-cols-3 '>
        ...
        <div className='grid grid-cols-7 col-span-2 items-center justify-center  text-center  '>
          ...
          <Cell>{''}</Cell>
          <span className='col-span-3 text-[#636363] font-bold'>{format(value, 'LLLL yyyy')}</span>
          <Cell>{''}</Cell>
          ...
          {Array.from({ length: numDays }).map((_, index) => {
            ...
            return (
              <Cell
                ...
                // className={`${selectedDay === date ? 'bg-[#F9E5E5] text-[#CF2A2A] font-bold' : ''}`}
                key={date}
              >
                {date}
              </Cell>
